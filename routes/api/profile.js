...
router.post(
  ...
  async (req, res) => {
    ...
    const { name, duration, severity, status, symptoms, goals } = req.body;
    ...
    if (status) profileFields.severity = severity;
    if (symptoms) profileFields.symptoms = symptoms;
...
router.put(
  ...
  [
    auth,
    [
      // check('specialist', 'specialist is required')
      //   .not()
      //   .isEmpty(),
